...
def main() -> None:
    ...
    st.caption("Final seven-variable logistic regression model")
    ...
    with calculator_tab:
        ...
        with col1:
            ...
            st.metric("Probability of 90-day unfavourable outcome (mRS 4-6)", f"{probability * 100:.1f}%")
            st.write(risk_band(probability))
            st.caption(f"Linear predictor: {eta:.3f}")

        with col2:
            st.subheader("Model transparency")
            st.write("Model type: final seven-variable logistic regression model")
            st.write("External validation AUC: 0.854")
            st.write("Outcome: 90-day unfavourable functional outcome, mRS 4-6")
            st.write("This calculator is intended for research-use risk estimation and local validation.")
